...
impl WebApp {
    pub fn new(config: WebConfig) -> Option<Self> {
        ...
        runner.provide_root_context(|| {
            ...
            global_contexts.insert_context(Clipboard::create(Some(
                Box::new(WebClipboard) as Box<dyn ClipboardProvider>
            )));
            global_contexts
        });
        ...
        let platform = runner.provide_root_context({
            ...
            move || Platform {
                ...
                // The canvas has no frame of its own: it is always at the origin, never filled
                // and never fullscreen as far as the app can tell.
